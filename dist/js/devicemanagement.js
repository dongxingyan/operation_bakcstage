...
function MeetingRoom(meetingroom_data){
		...
		this.guoqitime=meetingroom_data.expirationDate.substring(0,10)
		...
		this.li_org.className = "meetingroom-data11";
	...
	MeetingRoom.prototype.updateRoom = function(){
	    ...
	    // $("#add-a55").val(this.descriptions)
	    $("#user-start").html(this.sttime)
	    $(".a66").val(this.descriptionss)
	    $("#datatime9").val(this.guoqitime)
		nowId = this.id;
		nowId1=this.organId;
	...
	MeetingRoom.prototype.deleteRoom = function(){
		...
		$.ajax({
			type: 'delete',
			url:url+'cloudpServer/v1/orgs/device/'+deleteID+'/?token='+admin_token,
			success: function(data){
				console.log(data)
				if(data.code==0){
				$(".bcgs").show()
...
$.ajax({
		type: "get",
		url:url+'cloudpServer/v1/orgs/devices/?token='+admin_token,
		success: function(data){
			firstShowList(data);
		},
		error: function(erro){
		...
		$.ajax({
			...
			success: function(data){
				...
				$(".editor-user").hide()
									        	$(".alerts").show()
									        	$(".bcgs").show()
				$(".alert-content").html("没有符合条件的设备账号")
						$(".alert-sure").click(function(){
							$(".add-user").hide()
							location.reload()
						})
...
$(".true3").click(function(){
	var orgids=$("#user-org option:selected").val()
	          	 	var ps=$("#user-a4").val()
	          	 	var tm=$("#datatime9").val().substring(0,10)+' 00:00:00';
	          	 	var dess=$(".a66").val()
	          	 	var des=$("#add-a55").val()
	          	 	var nm=$("#user-a3").html()
	          	 	var c=/^\d{6}$/;
	          	 	...
	          	 	if(ps==''){
	          	 		...
	          	 		var data={
									  ...
									  pexipName:nm,
									  expirationDate:tm,
									  pexipDescription:dess
									  // description:des

	          	 			}
	          	 		console.log(data)
	          	 						var url1 =url+'cloudpServer/v1/orgs/'+nowId1+'/devices/'+nowId+'?token='+admin_token;
...
$(".title9").click(function(){
	$(".bcgs").show();
	$(".add-user").show()
	$.getJSON(url+'cloudpServer/v1/orgs/device/count/1/flag/1?token='+admin_token,function(data){
		   if(data.code==0){
					onecountroom=data.data[0].accountNum
						$(".device-other").val(onecountroom)
						$(".device-other1").val(onecountroom+"@cloudp.cc")
	...
	$(".true").click(function(){
		...
		var b=$(".device-other").val()
		var c=$(".device-other1").val()
		var d1=$("#datatime6").val()+':00'
		...
		var data={
	          	 			...
	          	 			console.log(data)
	          	 						var url1 = url+'cloudpServer/v1/orgs/device?token='+admin_token;
										var xmlhttp = new XMLHttpRequest();
...
$(".editor-right").click(function(){
	...
	$(".change-code").click(function(){
		...
		$.getJSON(url+'cloudpServer/v1/orgs/device/count/'+counta+'/flag/'+countb+'?token='+admin_token,function(data){
			console.log(data)
				if(data.code==5){
					$(".add-user").hide()
					$(".bcgs").show()
		...
		$(".trues").click(function(){
			...
			var d=$("#datatime4").val()+':00'
			var d1=$("#datatime5").val()+':00'
			var k=$("#add-a66").val()
			// var e=$(".count-password").val()
